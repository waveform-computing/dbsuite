...
Each module is expected to export a class called Input which, when constructed
...
Python ethos of not using dictionaries for database result sets, but tuples are
immutable and we need to convert some of the values (like timestamps).  It's
also useful as it enables us to use the inner dictionaries as keyword args when
constructing objects later.
...
hierarchy (in particular, see the cache for table dependencies, and the indexes
that apply to a given table).
"""
